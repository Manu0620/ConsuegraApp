...
export async function GET(req: NextRequest) {
   try {
      ...
      const page = Number.isNaN(parseInt(searchParams.get('page') || ''))
         ? 1
         : parseInt(searchParams.get('page') || '1');
      const limit = Number.isNaN(parseInt(searchParams.get('limit') || ''))
         ? 10
      ...
      const desde = searchParams.get('desde')
         ? parseFloat(searchParams.get('desde') || '0')
         : null;
      const hasta = searchParams.get('hasta')
         ? parseFloat(searchParams.get('hasta') || '0')
         : null;
