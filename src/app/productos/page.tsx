...
import { zodResolver } from '@hookform/resolvers/zod';import { filtersFormSchema } from '@/lib/utils';
...
export default function Products() {
  ...
  const createQueryParams = (filterData: FormData): URLSearchParams => {
    const params = new URLSearchParams(
      Object.entries(filterData).filter(([_, value]) => value !== undefined && value !== '')
    );
  ...
  const ProductsList = () => (
    <div className="product-list flex flex-row flex-wrap grow px-8 justify-center items-center w-full">
      {products.length > 1 ? (
        ...
        <h1 className="text-red-800 font-semibold text-2xl">😥 No se encontraron productos</h1>
      )}
    </div>
  );
            ...
            <>
              <div className='flex flex-row justify-between p-8 w-full'>
                <div className='flex flex-row items-center scale-95'>
                  ...
                  <p className='text-red-800 font-semibold px-2'> Page {page} of {pagination.totalPages}</p>
                  <button
                ...
                <div className='flex flex-row items-center w-'>
                  ...
                  <p className='text-red-800 font-normal text-[12px] w-1/2'>Showing {pagination.pageSize} of {pagination.totalProducts}</p>
                </div>
              </div>
              <ProductsList />
              <div className='flex flex-row justify-between p-8 w-full'>
                <p className='flex text-red-800 font-semibold text-[14px]'>Showing {pagination.pageSize} of {pagination.totalProducts}</p>
