...
export const ProductCard = (product: Products) => {
    ...
    const handleClick = () => {
        ...
        toast({
            ...
            description: (
                <p>
                    {product.description} ha sido agregado al carrito
                </p>
            ),
          })
    }
    ...
    return(
        <>
            <div key={product.id} className="flex flex-col space-y-1 basis-1/5 p-3 rounded-3xl hover:scale-105 hover:px-3 transition ease-linear duration-100">     
                <div className="card-content last:flex flex-col text-start space-y-1">
                    <Link className="flex flex-col w-full" href={`/productos/${product.id}`}>
