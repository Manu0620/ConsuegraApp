         ...
         <Dialog open={open}> 
            <DialogContent className="bg-red-50 text-red-800 border-red-800 rounded-3xl lg:w-3/12 md:w-6/12 sm:w-8/12 mobile:w-10/12 mobilesm:w-11/12">
               <DialogTitle className="flex flex-col gap-3 p-3 border-b text-red-800 text-lg font-bold items-center justify-center">
                  <BsFillSendCheckFill className="animate-in drop-shadow-lg" size={36} />
                     ...
                     <form onSubmit={onSubmit} className="flex flex-col gap-3 w-full">
                        <FormField
                            ...
                            render={({ field }) => (
                                ...
                                <FormItem className="basis-1/2">
                                    <FormLabel className="text-red-800 font-semibold">Código</FormLabel>
                                    <FormControl className="border-red-800 focus:bg-red-800/25 text-red-800 font-medium rounded-xl">
                                        <Input placeholder="Digite el código enviado..." {...field} />
                                    </FormControl>
                        ...
                        <div className="flex flex-row gap-3">
                           <Button 
                              ...
                              onClick={resendCode}>
                              {resending ? <LuLoader2 className="mr-2 animate-spin" /> : <TiArrowForward  className="mr-2"/>}  Reenviar código
                           </Button>
                           <Button 
                              type="submit" 
                              className="flex items-center justify-center text-red-800 text-center w-full px-4 py-2 font-bold text-sm border border-red-800 rounded-xl hover:bg-red-800/75 hover:text-white hover:scale-105 transition ease-linear duration-100 outline-none"
                              >
                              {loading ? <LuLoader2 className="mr-2 animate-spin" /> : <IoMdCheckmarkCircleOutline  className="mr-2"/>}  Verificar
                           </Button>
                        </div>
                        <p className="text-gray-700 text-[12px] font-normal text-start">
                           Si no recibiste el código, por favor revisa tu bandeja de spam o correos no deseados
                        </p>
                     </form>
                  </Form>
               </DialogDescription>
               ...
               <img src="/LogoMono.png" alt="Logo" className="absolute drop-shadow-lg right-3 bottom-3 w-8 "/>
            </DialogContent>
         </Dialog>
      </>
   )
